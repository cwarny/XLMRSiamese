...
class Wrapper(nn.Module):
    def __init__(self):
        super().__init__()
        self.model = Model()
        self.loss_func = SiameseLoss(margin=1.)
    
    def forward(self, x, y):
        logits,reps = self.model(x)
        print('Batch size in wrapper: %i' % logits.size(0))
        bs = reps.size(0)
        a = reps[:bs//2] # anchors
...
opt = Adam(wrapper.model.parameters(), lr=1e-3)

if args.fp16:
    if not is_apex_available():
        raise ImportError("Please install apex from https://www.github.com/nvidia/apex to use fp16 training.")
    model, opt = amp.initialize(wrapper.model, opt, opt_level='O1')
    wrapper.model = model

if args.parallel: wrapper = nn.DataParallel(wrapper)
...
loss = wrapper(xb, yb)
if args.fp16:
    with amp.scale_loss(loss, opt) as scaled_loss: scaled_loss.backward(torch.ones(device_count).to(device))
else: loss.backward(torch.ones(device_count).to(device))
opt.step()
opt.zero_grad()